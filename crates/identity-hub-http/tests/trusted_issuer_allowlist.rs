...
fn credential_offer_body(issuer_did: &str) -> Value {
    json!({
        "issuer": issuer_did,
        // A full (non-sparse) CredentialObject - this file exercises the
        // trusted-issuer allow-list, not `crate::validation`'s own content
        // checks (see message_content_validation.rs for those), so a
        // self-describing entry avoids this fixture depending on a
        // resolvable catalog.
        "credentials": [
